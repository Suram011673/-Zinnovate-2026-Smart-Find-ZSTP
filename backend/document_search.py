...
def _slice_block_bbox_by_char_span(b: dict[str, Any], text: str, c0: int, c1_excl: int) -> list[float]:
    """Horizontal slice of block bbox for character range [c0, c1_excl) in ``text``."""
    n = max(len(text), 1)
...
def search_ocr_blocks(blocks: list[dict[str, Any]], query: str) -> list[dict[str, Any]]:
    ...
    # Dynamic guard: for any single-word query, remove broad line-style matches so
    # highlights stay focused on specific words/boxes.
    if single_word_query:
        out = [
            m
            for m in out
            if str(m.get("match_type") or "")
            not in {"line", "line_nospace", "line_substring", "line_digits", "fuzzy_line", "fuzzy_line_tokens"}
        ]
